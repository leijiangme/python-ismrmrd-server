...
def process_raw(group, connection, config, metadata):
    ...
    if math.floor(full_x_sample / 2.0) == x_center_sample:
        readout_num = group[0].data.shape[0]
    else:
        readout_num = full_x_matrix

    coil_num = group[0].data.shape[1]
    ...
    k_space_matrix = np.zeros((readout_num, coil_num, PE_num), group[0].data.dtype)

    # Determine the indexing range based on x_center_sample condition
    if math.floor(full_x_sample / 2.0) == x_center_sample:
        row_range = np.arange(0, readout_num)
    else:
        row_range = np.arange(int(full_x_matrix / 2.0 - x_center_sample) - 1, full_x_matrix)

    # Reconstruct images
    images = []

    # Loop through each acquisition and add it to the correct k-space matrix
    for i in np.size(group,0):
        acq = group[i]
    ...
    ksp = np.transpose(k_space_matrix, (1, 3, 2))

    # Fourier Transform
    # Fourier Transform
    ksp = fft.fftshift(ksp, axes=(1, 2))
    ksp = fft.ifft2(ksp, axes=(1, 2))
    ksp = fft.ifftshift(ksp, axes=(1, 2))
    ksp *= np.prod(ksp.shape)  # FFT scaling for consistency with ICE
